...
* Full Steps for Deployment
  #+BEGIN_SRC bash
  git clone https://github.com/chenxiaoyu233/Optimization
  cd Optimization/k-defective
  git checkout speed-up
  tar -xzvf glpk-4.65.tar.gz
  mv glpk-4.65 glpk
  bash MyInstall.sh    #安装glpk, 所有文件都安装在 glpk/ 中
  cmake -DNDEBUG=on .  #有些环境需要调整cmake (集群上需要打开 -static 选项)
  make && make install #会在当前目录下和/Debug目录下生成可执行文件
  #+END_SRC
