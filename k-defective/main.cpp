...
void solve(string solverT, string filePath, int k) {
	...
	while(fscanf(in, " %c", &flag) != EOF){
		if(flag == 'c') {
			...
			int n, m;
			fscanf(in, " %*s%d%d", &n, &m);
			if (solverT == "algo") solver = new MyBitSetImplement<MyBitSet<128>, KDefectiveBase>(n);
			else if (solverT == "bao") solver = new BaoSolver(n);
            else if (solverT == "RDS") solver = new BitSetImplement<bitset<128>, KDefectiveRDS>(n);
            else if (solverT == "Simple") solver = new MyBitSetImplement<MyBitSet<128>, KDefectiveBase>(n);
		}else if(flag == 'e') { // edge
